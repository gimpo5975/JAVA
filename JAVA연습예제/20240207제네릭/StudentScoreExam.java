...
public class StudentScoreExam {
	...
	public static void main(String[] args) {
		...
		System.out.println("수학 최고 점수" + findBest2(mathArr));
	...
	public static EnglishScore findBest(EnglishScore[] engArr) {
		...
		EnglishScore max = engArr[0]; // 최고 점수 찾기
	...
	// 최고 점수를 찾는 메소드
	public static MathScore findBest2(MathScore[] engArr) {

		MathScore max = engArr[0]; // 최고 점수 찾기
		for (int i = 0; i < engArr.length; i++) {
			if (max.compareTo(engArr[i]) < 0) {
				max = engArr[i];
			}
